...
struct tunit_test
{
	...
	static void load()
	{
		...
		BOOST_CHECK_EQUAL(tmoon::load(input).id_, result.id_);
		BOOST_CHECK_EQUAL(tmoon::load(input).origin_.x, result.origin_.x);
		BOOST_CHECK_EQUAL(tmoon::load(input).origin_.y, result.origin_.y);
		BOOST_CHECK_EQUAL(tmoon::load(input).origin_.z, result.origin_.z);
		BOOST_CHECK_EQUAL(tmoon::load(input).orbit_.rotation,
						  result.orbit_.rotation);
		BOOST_CHECK_EQUAL(tmoon::load(input).orbit_.rotation_period_theta_plane,
						  result.orbit_.rotation_period_theta_plane);
		BOOST_CHECK_EQUAL(tmoon::load(input).orbit_.rotation_period_phi_plane,
						  result.orbit_.rotation_period_phi_plane);
		BOOST_CHECK_EQUAL(tmoon::load(input).radius_, result.radius_);

		input.put("version", "100");
		CHECK_EXCEPTION_MESSAGE(tmoon::load(input),
								load_invalid_version,
								"Failed to load a moon object, version »100« "
								"is not supported.");
	}
...
BOOST_AUTO_TEST_CASE(test_modules_celestial_moon)
{
	tunit_test::load();
	tunit_test::store();
}
